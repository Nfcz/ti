...
for a in range(len(lst)- 2):
  ...
  if (lst[a], lst[a+1]) not in t:
    t[(lst[a], lst[a+1])]= [[lst[a+2], 1]]



  else:
    for r in range(len(t[(lst[a], lst[a+1])])):
      
      if t[(lst[a], lst[a+1])][r][0]== lst[a+2]:
          t[(lst[a], lst[a+1])][r][1]+= 1
          h=1
    if h==0:
      t[(lst[a], lst[a+1])].append([lst[a+2], 1])
...
for q in range(sizelern):
  e= random.choice(list(t))
  rand[e]= t[e]

#Ксожелению из-за маленькой выборки сэмплирование не позволяет написать текст т к не после всех сочетаний есть варианты

rand= t


sizetext= 50 #количество слов в получаемом тексте

text= ['']*sizetext

randfirst= random.choice(list(rand)) #рандомные первые слова

text[0] = randfirst[0]
text[1] = randfirst[1]

#Дальше подбираем слова постфиксы с наибольшей частотностью
for e in range(sizetext-2):
  
  u= 0
  a= ''
  if (text[e], text[e+1] ) in rand:
    for i in range(len(rand[(text[e], text[e+1] )])):
       if u < rand[(text[e], text[e+1] )][i][1]:
          a= rand[(text[e], text[e+1] )][i][0]

    text[e+2]= a 
  else:
    text[e+2]= random.choice(list(rand))[0]
...
#Можно заметить, что при большом количестве слов часто идут циклические фразы, как в подсказках на телефоне)
print(strtext)
...
randfirst= random.choice(list(rand))
